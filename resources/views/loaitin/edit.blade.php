...
@section('content')
<form action="{{ route('loaitin.update', ['id' => $theloai->id]) }}" method="post">
  {{ csrf_field() }}
  {{ method_field('put') }}
  ...
  <div class="form-group">
    <label for="exampleInputEmail1">theloai_id</label>
    <input name="theloai_id" type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Enter Order">
    <small id="emailHelp" class="form-text text-muted">We'll never share your email with anyone else.</small>
  </div>
